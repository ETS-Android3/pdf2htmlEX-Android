...
// make_unique backport to C++11 by Herb Sutter
...
namespace pdf2htmlEX {
  ...
  pdf2htmlEX::pdf2htmlEX() : param(make_unique<Param>()) {
  ...
  public:
    GlobalParamsGC(const char *popplerDataDir) {
      ...
      m_globalParams = make_unique<GlobalParams>(popplerDataDir);
  ...
  void pdf2htmlEX::convert() {
    ...
    if (!param->owner_password.empty()) {
      ownerPW = make_unique<GooString>(param->owner_password);
    }

    std::unique_ptr<GooString> userPW;
    if (!param->user_password.empty()) {
      userPW = make_unique<GooString>(param->user_password);
    }
    ...
    HTMLRenderer(*param).process(doc.get());
  }
